...
export interface WasteCollectReqElement {
  position: number;
  name: string;
  quantity: number;
  unit: string;
...
const WASTE_COLLECT_REQ_DATA: WasteCollectReqElement[] = [
...
const ATTACHMENTS_DATA: AttachmentElement[] = [
  { position: 1, wasteType: 'نفايات صلبة', entity: 'هيئة السلامة و إدارة البيئة',
     fileName: 'report-01.pdf', fileType: 'PDF', uploadedBy: '', actions: 'edit' },
  { position: 2, wasteType: 'نفايات سائلة', entity: 'إدارة البيئة',
     fileName: 'photo-incident.jpg', fileType: 'JPG', uploadedBy: '', actions: 'edit' },
];

@Component({
  selector: 'app-waste-collect-req',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, MatFormFieldModule, MatRadioModule, MatSelectModule, MatTableModule],
  templateUrl: './waste-collect-req.html',
  styleUrl: './waste-collect-req.css',
})
export class WasteCollectReq {
  selectedTab: 'wastes' | 'attachments' = 'wastes';
  displayedColumns: string[] = ['position', 'name', 'quantity', 'unit', 'company', 'actions'];
  dataSource = WASTE_COLLECT_REQ_DATA;
  attachmentsDisplayedColumns: string[] = ['position', 'wasteType', 'entity', 'fileName', 'fileType', 'uploadedBy', 'actions'];
  attachmentsDataSource = ATTACHMENTS_DATA;
