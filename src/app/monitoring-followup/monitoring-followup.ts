...
export type CulpritType = 'vessel' | 'company' | 'truck' | 'unknown' | 'natural';

@Component({
  selector: 'app-monitoring-followup',
  standalone: true,
  imports: [ReactiveFormsModule, MatFormFieldModule, MatRadioModule, MatSelectModule],
  templateUrl: './monitoring-followup.html',
  styleUrl: './monitoring-followup.css',
})
export class MonitoringFollowup {
  selectedCulpritType: CulpritType = 'unknown';
  sourceValue: 'device' | 'station' | 'report' = 'report';

  readonly saveClicked = output<void>();
  readonly cancelClicked = output<void>();
  readonly createStatementClicked = output<void>();
  ...
  protected onCulpritTypeChange(event: MatRadioChange): void {
    this.selectedCulpritType = event.value as CulpritType;
  }
  ...
  protected onCreateStatementClick(): void {
    this.createStatementClicked.emit();
  }
